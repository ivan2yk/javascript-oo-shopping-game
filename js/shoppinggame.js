...
class Rating {
    ...
    set rating(inputRate) {
        this.rate = 'BAD';
        if (inputRate > 1 && inputRate <= 4) {
            this.rate = 'OK';
        }
        if (inputRate >= 5 && inputRate <= 7) {
            this.rate = 'GOOD';
        }
        if (inputRate > 7) {
...
const loadProducts = (map, prodId) => {
    ...
    try {
        ...
        let prodKeys = Object.keys(Product.prototype);
...
const loadMagicProducts = (map, prodId) => {
    ...
    try {
        ...
        let magProdKeys = Object.keys(MagicProduct.prototype);
